...
from .position_counting_automaton import CounterVector
...
class SuperConfig(SuperConfigBase):
    ...
    def update(self, symbol) -> "SuperConfig":
        assert len(symbol) == 1

        next_super_config = dd[State, OrderedSet[CounterVector]](OrderedSet)
        for config in self:
...
class CounterConfig(SuperConfigBase):
    ...
    def __init__(self, automaton: PositionCountingAutomaton):
        super().__init__(automaton)
        self.configs: SuperConfigBase = dd(OrderedSet)

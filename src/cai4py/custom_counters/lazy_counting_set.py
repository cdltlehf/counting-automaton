...
class LazyCountingSet(CounterBase):
    ...
    def __init__(self, lower_bound: int, upper_bound: int) -> "LazyCountingSet":
        ...
        LazyCountingSet._data_collection.max_memory_update(sys.getsizeof(self.list))

    def inc(self):

        LazyCountingSet._data_collection.full_update(Operation.INC)
        LazyCountingSet._data_collection.half_update(len(self.list) // 2, Operation.INC) # Account for lambda function
    ...
    @staticmethod
    def union(set1: "LazyCountingSet", set2: "LazyCountingSet") -> "LazyCountingSet":

        LazyCountingSet._data_collection.full_update(Operation.UNION)
        LazyCountingSet._data_collection.half_update(3*len(set1.list), Operation.UNION)
        ...
        if set1.upper_bound == -1:
            LazyCountingSet._data_collection.half_update(2*len(setUnion), Operation.UNION)
        ...
        LazyCountingSet._data_collection.max_memory_update(sys.getsizeof(inst.list))
    ...
    def ge_lower_bound(self) -> bool:
        ...
        LazyCountingSet._data_collection.half_update(len(self.list), Operation.GE)
    ...
    def le_upper_bound(self) -> bool:
        ...
        LazyCountingSet._data_collection.half_update(len(self.list), Operation.LE)

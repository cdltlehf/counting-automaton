...
class CountingSet(CounterBase):
    ...
    def __init__(self, lower_bound: int, upper_bound: int) -> "CountingSet":
        ...
        CountingSet._data_collection.max_memory_update(sys.getsizeof(self.queue))
    ...
    @staticmethod
    def union(counter_1: "CountingSet", counter_2: "CountingSet") -> "CountingSet":
        ...
        while len(counter_1.queue) != 0 and len(counter_2.queue) != 0:
            ...
            if counter_1.queue[0] + counter_1.offset > counter_2.queue[0] + counter_2.offset: # >
                aux.append(counter_1.queue.popleft() + counter_1.offset)
            elif counter_1.queue[0] + counter_1.offset < counter_2.queue[0] + counter_2.offset: # <
        ...
        CountingSet._data_collection.half_update(len(leftover_counter.queue)//2, Operation.UNION)
        while len(leftover_counter.queue) != 0:
            aux.append(leftover_counter.queue.popleft() + leftover_counter.offset)
        ...
        CountingSet._data_collection.max_memory_update(sys.getsizeof(new_counter.queue))

"""Plot the matching time of configuration representation."""
...
def results_to_arrays(
    ...
    xs: list[float] = []
    time_perf_list: list[int] = []
    time_process_list: list[int] = []
...
def main(
    ...
    ############################################################################
    _, upper_bound_x = get_outlier_bounds(xs_total)
    _, upper_bound_y = get_outlier_bounds(ys_total)
    ############################################################################
    # _, upper_bound_x = get_outlier_bounds(np.emath.power(base, xs_total))
    # upper_bound_x = np.emath.logn(base, upper_bound_x)
    # _, upper_bound_y = get_outlier_bounds(np.emath.power(base, ys_total))
    # upper_bound_y = np.emath.logn(base, upper_bound_y)
    ############################################################################
    upper_bound = max(upper_bound_x, upper_bound_y)

    xs_outlier = (xs_total >= upper_bound) & ~xs_timeout
    ys_outlier = (ys_total >= upper_bound) & ~ys_timeout
    ...
    ############################################################################
    # xbins = np.logspace(1, np.emath.logn(base, upper_bound), 11, base=base)
    # steps = xbins[1] / xbins[0]
    # x_max = xbins[-1]
    # xbins = np.append(xbins, [x_max * steps, x_max * steps * steps])
    ############################################################################
    ybins = xbins
    bins = (xbins, ybins)
    ...
    xs = xs_total.copy()
    ys = ys_total.copy()
    xs[xs_outlier] = (xbins[-3] + xbins[-2]) / 2
    ys[ys_outlier] = (ybins[-3] + ybins[-2]) / 2
    xs[xs_timeout] = (xbins[-2] + xbins[-1]) / 2
    ys[ys_timeout] = (ybins[-2] + ybins[-1]) / 2
